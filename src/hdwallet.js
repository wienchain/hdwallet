...
HDWallet.createNewKey = function (network, pass, progressCallback) {
  ...
  network = (network === 'testnet' ? wienchainTestnetNetwork : wienchainMainnetNetwork)
  ...
  // coin_type'
  node = node.deriveHardened(network === wienchainMainnetNetwork ? 0 : 1)
  // account'
...
HDWallet.getInputAddresses = function (txHex, network) {
  network = network || wienchainMainnetNetwork
  var addresses = []
  var tx
  try {
...
HDWallet.prototype.getKeyPrefix = function () {
  ...
  var network = (self.network === wienchainMainnetNetwork) ? 'mainnet' : 'testnet'
...
HDWallet.prototype.registerAddress = function (address, accountIndex, addressIndex, change) {
  ...
  var coinType = self.network === wienchainMainnetNetwork ? 0 : 1
  change = (change) ? 1 : 0
...
HDWallet.prototype.deriveAccount = function (accountIndex) {
  ...
  if (this.preAccountNode) {
    ...
    node = node.deriveHardened(44)
    // coin_type'
    node = node.deriveHardened(this.network === wienchainMainnetNetwork ? 0 : 1)
